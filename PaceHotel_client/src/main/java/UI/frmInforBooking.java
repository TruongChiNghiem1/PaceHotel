...
//			inputGender.setText(Integer.toString(guest.getGender()));
			...
			inputNumOfAdults.setText(bookingItem[1].toString());
			inputNumOfChildren.setText(bookingItem[2].toString());
			if (bookingItem[9] != null) {
				LocalDateTime bookingDateTime = (LocalDateTime) bookingItem[9];
			    String formattedDateBooking = bookingDateTime.format(formatter);
			    inputTimeOfBooking.setText(formattedDateBooking);
			}
			if (bookingItem[7] != null) {
			...
			if (bookingItem[8] != null) {
			    ...
			    String formattedDeinputarture = deinputarture.format(formatter);
			    inputTimeOfDeinputarture.setText(formattedDeinputarture);
			}
			aNote.setText(bookingItem[3].toString());
		}
	}
...
//		JOptionPane.showConfirmDialog(null, guestList.findGuest(indentityCard));
	...
	public void CheckIn() throws ParseException, RemoteException {
		String inputTimeOfBooking1 = inputTimeOfBooking.getText();
		String indentityCard = inputCard.getText();
		Booking idBookingGet;
		Guest newGuest = null;
		int idGuest;
		...
		if (inputTimeOfBooking1.equalsIgnoreCase("")) {
			...
			if (guestList.findGuest(indentityCard) == -1) {
				...
				newGuest = new Guest(indentityCard, inputName1, inputDOB1, inputGender1, inputPhone1, inputEmail1,
			...
			Room roomId = roomList.findRoomByRoomNo(idRoom);
			Employee employeeId = employee_dao.layNhanVienTheoMa(maNhanVien);
			newGuest = guestList.getOneGuest(indentityCard);
			Booking newBooking = new Booking(1, inputNumOfAdults1, inputNumOfChildren1, aNote1, null, today, null, roomId, employeeId, newGuest);
			bookingList.addBooking(newBooking);
			
			idBookingGet = newBooking;
		//Nếu order trước
		} else {
			int idBookingGet1 = bookingList.findRoomBooking(idRoom);
			idBookingGet = bookingList.getOneBooking1(idBookingGet1);
			bookingList.updateBookingToArr(idBookingGet1);
		}
		
		Double discount = Double.parseDouble(inputDiscount.getText());
		
		Bill billEntity = new Bill(0,0, null, discount, newGuest);
		...
		BillDetail billDetail = new BillDetail(0, 0, billEntity, idBookingGet, null);
		billDetailList.addBillDetail(billDetail);
		
		JOptionPane.showMessageDialog(null, "Checkin successfull!");

		roomList.updateRoomStatus(idRoom, "Occupied");

		this.setVisible(false);
	}

	public void OrderBefore() throws ParseException, RemoteException {
		String indentityCard = inputCard.getText();
		if (guestList.findGuest(indentityCard) == -1) {
			String inputName1 = inputName.getText();
			int inputGender1 = cbGender.getSelectedIndex();
		...
		Room roomId = roomList.findRoomByRoomNo(idRoom);
		Employee employeeId = employee_dao.layNhanVienTheoMa(maNhanVien);
		Guest GuestId = guestList.getOneGuest(indentityCard);
		
		LocalDateTime today = LocalDateTime.now();
		
		Booking newBooking = new Booking(1, inputNumOfAdults1, inputNumOfChildren1, aNote1, today, null, null, roomId, employeeId, GuestId);
		
		bookingList.addOrder(newBooking);

		JOptionPane.showMessageDialog(null, "Order successfull!");

		roomList.updateRoomStatus(idRoom, "Ordered");
	...
	public void CheckOut() throws ParseException, RemoteException, NotBoundException {
		int idBookingGet = 0;
		...
		if(!inputTimeOfArr1.equalsIgnoreCase("")) {
			...
			bill = new BillUI(bookingItem, registryGlo);
			bill.setVisible(true);
			
			
		} else {
			JOptionPane.showMessageDialog(null, "room is not checkout");
		}
		
	}
	
	public void Ready() throws RemoteException {
		roomList.updateRoomStatus(idRoom, "Ready");
		this.setVisible(false);
	}
